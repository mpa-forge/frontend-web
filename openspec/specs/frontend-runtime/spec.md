...
`VITE_CLERK_SIGN_UP_FALLBACK_REDIRECT_URL`, plus the optional browser-safe
observability inputs `VITE_OBSERVABILITY_ENABLED`,
`VITE_OBSERVABILITY_ENDPOINT`, `VITE_OBSERVABILITY_TRANSPORT`, and
...
- **THEN** the required frontend runtime variables, the optional Clerk route
  placeholders, and the optional browser-safe observability values are listed
  as the local configuration template

#### Scenario: Browser observability config remains browser-safe

- **WHEN** a developer configures frontend observability through the documented
  runtime variables
- **THEN** the contract exposes only browser-safe release and ingest values and
  does not require committed secrets, tokens, or prebuilt auth headers
